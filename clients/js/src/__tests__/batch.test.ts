...
describe('batchKey', () => {
  it('groups the same device and separates a different one', () => {
    expect(batchKey({ platform: 'ios', country: 'FR' })).toBe(
      batchKey({ country: 'FR', platform: 'ios' })
    );
    expect(batchKey({ platform: 'ios' })).not.toBe(batchKey({ platform: 'ios', build: '42' }));
  });
